...
class LoginFlowConflict(RuntimeError):
    pass


def _describe_register_error(error: object) -> str:
    text = str(error or "")
    match = re.search(r"(?:_http_|sentinel_req_failed_)(\d{3})", text)
    if not match:
        return ""
    code = match.group(1)
    meanings = {
        "400": "请求被拒，常见原因是邮箱域名被风控、验证码错误或过期、注册资料/风控参数不被接受",
        "401": "认证失败，通常是登录态、验证码或 token 相关校验没有通过",
        "403": "访问被拒，通常和代理 IP、风控校验或 Sentinel 校验有关",
        "409": "流程状态冲突或登录 session 失效，系统会重建 session/device/PKCE 后重试",
        "429": "请求过于频繁或触发限流，建议降低线程数、更换代理或稍后重试",
        "500": "OpenAI 服务端异常或上游临时错误",
        "502": "上游网关异常，通常是临时网络或服务问题",
        "503": "上游服务暂不可用，通常稍后重试",
        "504": "上游响应超时，通常和网络、代理或服务拥堵有关",
    }
    stages = [
        ("platform_authorize", "初始化授权"),
        ("user_register", "提交邮箱和密码注册"),
        ("send_otp", "发送邮箱验证码"),
        ("validate_otp", "校验邮箱验证码"),
        ("create_account", "创建账号资料"),
        ("platform_login_authorize", "独立登录授权"),
        ("email_submit", "独立登录提交邮箱"),
        ("password_verify", "独立登录校验密码"),
        ("sentinel_req", "Sentinel 风控校验"),
    ]
    stage = next((label for key, label in stages if key in text), "未知阶段")
    return f"错误码: HTTP {code}；阶段: {stage}；含义: {meanings.get(code, '上游返回的 HTTP 错误，需要结合详细响应判断')}"
    ...
    def _login_and_exchange_tokens(self, email: str, password: str, mailbox: dict, index: int) -> dict:
        ...
        for attempt in range(1, 4):
            if attempt > 1:
                ...
                step(index, f"409 状态冲突，重建 session/device/PKCE 后重新登录换 token({attempt}/3)", "yellow")
            ...
            def _submit_email() -> None:
                ...
                if resp is not None and resp.status_code == 409:
                    raise LoginFlowConflict("email_submit_http_409")
                if resp is None or resp.status_code != 200:
                    data = _response_json(resp) if resp is not None else {}
            ...
            def _verify_password() -> None:
                ...
                if resp is not None and resp.status_code == 409:
                    raise LoginFlowConflict("password_verify_http_409")
                if resp is None or resp.status_code != 200:
                    body = ""
                    try:
                        body = (resp.text or "")[:500] if resp is not None else ""
            ...
            try:
                ...
                return tokens
            except LoginFlowConflict as exc:
                last_error = exc
                if attempt >= 3:
        ...
        raise RuntimeError(str(last_error or "独立登录换 token 失败"))
...
def worker(index: int) -> dict:
    ...
    try:
        ...
        with stats_lock:
            stats["done"] += 1
            stats["fail"] += 1
        error_hint = _describe_register_error(e)
        suffix = f"；{error_hint}" if error_hint else ""
        log(f"任务{index} 注册失败，本次耗时{cost:.1f}s，原因: {e}{suffix}", "red")

...
class OpenAIBackendAPI:
    ...
    def _poll_image_results(self, conversation_id: str, timeout_secs: float = 120.0) -> tuple[list[str], list[str]]:
        ...
        - Sleeps image_poll_initial_wait_secs first (default 20s, +jitter). ChatGPT
        ...
        - Subsequent polls are image_poll_interval_secs apart (default 8s).
        ...
        while _remaining() > 0:
            ...
            try:
                ...
                text = str(exc)
                retryable = any(f"status={code}" in text for code in (429, 500, 502, 503, 504))
                if retryable and _retry_sleep("upstream_status_text", None, text, None):
                    continue
                raise
    ...
    def resolve_conversation_image_urls(
        ...
        if urls or not poll or not conversation_id:
            return urls

        logger.info({
            "event": "image_resolve_retry_poll",
            "conversation_id": conversation_id,
            "file_ids": file_ids,
            "sediment_ids": sediment_ids,
        })
